...
namespace FrameDrag{
	...
	Vector3f ZYXEulerToAngularVelocity(const Vector3f& euler, 
		...
		//         droll/dt*(R_z)(R_y)x
		//
		//		(    0    )   ( -sin(yaw)*dpitch/dt )   (cos(yaw)*cos(pitch)*droll/dt)
		// ω =  (    0    ) + ( cos(yaw)*dpitch/dt  ) + (sin(yaw)*cos(pitch)*droll/dt)
	    ...
	    const auto cosp = std::cos(pitch);
	    const auto sinp = std::sin(pitch);
		return fromMatrix({0.0f, -siny,   cosy*cosp, 
		                   0.0f,  cosy,   siny*cosp,
		                   1.0f,  0.0f,  -sinp      }).apply(euler_derivative);
	}
	
    Vector3f ZYXEulerToAngularVelocity(float yaw, float pitch, float roll, 
                                       float yaw_deriv, float pitch_deriv, float roll_deriv){
	    return ZYXEulerToAngularVelocity(Vector3f{yaw, pitch, roll}, Vector3f{yaw, pitch, roll});
	}
}