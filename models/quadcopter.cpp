    ...
    const Vector3f& drag, const Vector3f& target_torque)
{
    ...
    auto linear_acceleration = (1.0f / _mass) * (gravity + _bodyframe_to_world.apply(Vector3f{ 0.0f, 0.0f, thrust }) + drag);
    auto angular_velocity = ZYXEulerToBodyFrameAngularVelocity(_euler_angles, _euler_derivatives);

    auto angular_acceleration = _I.inverse() * (torque - angular_velocity.cross(_I * angular_velocity));
