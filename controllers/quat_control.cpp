    ...
    const Quaternion& q_target_derivative)
{
    ...
    auto target_angular_velocity = QuaternionToBodyFrameAngularVelocity(q_target, q_target_derivative);
    auto angular_velocity_error = current_angular_velocity - q_error.inverse().apply(target_angular_velocity);

    auto target_acc = -_K_omega * angular_velocity_error - _K_R * orientation_error;

    return target_acc + current_angular_velocity.cross(_I * current_angular_velocity);
}
